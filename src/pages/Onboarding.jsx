...
// Doctor category → allowed consultation fee range (NGN)
// NOTE: assumed GP = lower range, Specialist = higher range. Flip min/max below if that's backwards.
const FEE_RANGES = {
    gp: { label: "General Practitioner (GP)", min: 2000, max: 6000 },
    specialist: { label: "Specialist (SP)", min: 4000, max: 8000 },
};

const FEE_CATEGORY_OPTIONS = [
    { value: "gp", label: "General Practitioner (GP)" },
    { value: "specialist", label: "Specialist (SP)" },
];
...
const INIT = {
    ...
    licenseNumber: "", licenseExpiry: "",
    availabilitySchedule: [],
    feeCategory: "", consultationFee: "", currency: "NGN",
    certificateUrl: "",
};
...
const Onboarding = ({ route }) => {
    ...
    const runValidation = () => {
        // ── Step 7 (Pricing) needs custom logic: category + range-aware fee check ──
        if (step === 7) {
            const errs = {};

            if (!formData.feeCategory) {
                errs.feeCategory = "Please select a category";
            }

            const feeStr = String(formData.consultationFee ?? "").trim();
            const fee = Number(feeStr);

            if (!feeStr) {
                errs.consultationFee = "This field is required";
            } else if (isNaN(fee)) {
                errs.consultationFee = "Must be a valid number";
            } else if (formData.feeCategory) {
                const range = FEE_RANGES[formData.feeCategory];
                if (fee < range.min || fee > range.max) {
                    errs.consultationFee = `Fee must be between ₦${range.min.toLocaleString()} and ₦${range.max.toLocaleString()} for ${range.label}`;
                }
            }
    ...
    const renderPricingStep = () => {
        ...
        return (
            <>
                {field("feeCategory", "Doctor Category", {
                    type: "select",
                    ...
                    <div className="ob-fee-wrap">
                        ...
                        <input
                            ...
                            onChange={e => {
                                ...
                                setErrors(p => ({ ...p, consultationFee: "" }));
                            }}
                        />
                    </div>
                    <p style={{ fontSize: "0.75rem", color: "var(--soft)", marginTop: "4px" }}>
                        {range
                            ? `${range.label} fees must be between ₦${range.min.toLocaleString()} and ₦${range.max.toLocaleString()}.`
                            : "Select a category above to see the allowed fee range."}
                    </p>
                    {errors.consultationFee && (
                        <span className="ob-error-msg">{Icon.info} {errors.consultationFee}</span>
                    )}
                </div>
            </>
        );
            ...
            <div className="ob-review-section">
                ...
                <div className="ob-review-grid">
                    ...
                    <RenderReviewItem
                        ...
                        value={FEE_RANGES[formData.feeCategory]?.label}
                    />
                    <RenderReviewItem
                        label="Consultation Fee"
                        value={formData.consultationFee ? `₦${Number(formData.consultationFee).toLocaleString()}` : "Free"}
